...
actualizar_proveedor(IdProveedor, NuevoNombre, NuevoContacto, NuevoTelefono) :-
    conectar,
    format(atom(Query), 'UPDATE proveedores SET nombre=\'~w\', contacto=\'~w\', telefono=\'~w\' WHERE id_proveedor = ~w', [NuevoNombre, NuevoContacto, NuevoTelefono, IdProveedor]),
    odbc_query(conexion, Query, _),
    desconectar.
...
consultar_todos_proveedores(Proveedores) :-
    ...
    findall(proveedor(Id, Nombre, Contacto, Telefono),
            odbc_query(conexion, 'SELECT id_proveedor, nombre, contacto, telefono FROM proveedores',
